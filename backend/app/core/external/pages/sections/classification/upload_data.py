...
import flask
import uuid
from app.core.external.pages.sections.classification.utils import session_delete_file, parse_contents, render_upload_header, reset_button, continue_button, session_df_to_file, session_schema_create, render_upload_header_database, get_existing_well_list, get_well_data_list, get_well_data, create_session
...
upload_button_database = dmc.Button(
    id='classification-upload-data-database',
    children=dmc.Stack([
        dmc.Flex(DashIconify(icon="ic:outline-cloud-upload", height=30), justify='center'),
        ...
        [   
            ...
            html.Div(
                dmc.Select(
                    ...
                    value='file',
                    id='classification-input-select',
                    data = [
            ...
            dcc.Loading(type='circle',id='classification-loading-upload_data',style={"width": "80px", "height": "80px"}),
            dmc.Modal(id='classification-modal-database', opened=False),
            html.Div(
                id='classification-output-data'
            ),
            html.Div(
                id='classification-alert-delete'
            ), 
            dmc.Group(
                [
                    reset_button,
                    html.Div(id='classification-proceed-output')
                ],
...
@callback(
    ...
    Input('classification-input-select','value'),
...
@callback(
    ...
    Output('classification-upload-header', 'children', allow_duplicate=True),
    Output('classification-proceed-output', 'children', allow_duplicate=True),
    Output('classification-loading-upload_data','children',allow_duplicate=True),
    Output('classification-alert-delete','children',allow_duplicate=True),
...
def upload_data_processing(contents, filename):
    if contents is not None:
        try:
            ...
            check = df.select_dtypes(exclude=[np.number]).columns

            if len(check) < 1:
                upload_output = dmc.Alert(
                    'Input for Classification need at least one categorical column',
                    color="red",
                    variant="filled",
                    withCloseButton=True
                )
...
@callback(
    Output('classification-modal-database', 'opened'),
    Output('classification-modal-database', 'children'),
    Output('classification-output-data','children',allow_duplicate=True),
    Input('classification-upload-data-database', 'n_clicks'),
    [State('classification-input-select', 'value')],
    prevent_initial_call=True
)
def open_modal(n_clicks, selected_value): 
    if selected_value == 'database' and n_clicks > 0:
        ...
        dmc.Space(h=20),
        dmc.Button("Apply", color="blue", id='classification-database-submit', n_clicks=0),]
        
        # Modal dengan form yang baru dibuat
        modal_database = dmc.Modal(
            title="Are you sure you want to reset?",
            id='classification-modal-database',
        )
...
@callback(
    ...
    Output('classification-proceed-output', 'children', allow_duplicate=True),
    Output('classification-loading-upload_data','children',allow_duplicate=True),
    Output('classification-modal-database', 'opened', allow_duplicate=True),  # Close modal here
    Output('classification-alert-delete','children',allow_duplicate=True),
    Input('classification-database-submit','n_clicks'),
...
def upload_data_processing_database(n_clicks,form_data):
    if n_clicks > 0:
        ...
        if get_dataframe is None:
            ...
            # Ensure all five outputs are returned
            return upload_output, no_update, no_update, "", False, no_update
        
        else:
            turn_dataframe = pd.DataFrame(get_dataframe)
            upload_header = render_upload_header_database(f"database-{form_data['well_data_type']}", 'classification-rawData')
            ...
            check = turn_dataframe.select_dtypes(exclude=[np.number]).columns
            
            if len(check) < 1:
                upload_output = dmc.Alert(
                    'Input for Classification needs at least one categorical column',
                    color="red",
                    variant="filled",
                    withCloseButton=True
                )
                # Return all five outputs
                return upload_output, no_update, no_update, "", False, alert_delete
            
            else:
                # Format dataframe display
                turn_dataframe = turn_dataframe.head(5).round(2)
                upload_output = html.Div(
                    dmc.Table(
                        striped=True,
                        highlightOnHover=True,
                ...
                continue_button = dmc.Button("Proceed", id="classification-proceed-button")
                
                # Return all five outputs
                return upload_output, upload_header, continue_button, "", False, alert_delete
    
    # If n_clicks is not greater than 0, return initial state
...
@callback(
    ...
    Output('classification-upload-header', 'children',allow_duplicate=True),
    Output('classification-proceed-output','children',allow_duplicate=True),
    Input('classification-rawData-remove-data-database', 'n_clicks'),
    prevent_initial_call=True
)
