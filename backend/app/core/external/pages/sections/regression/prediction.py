...
from app.core.external.pages.sections.regression.utils import parse_validation_errors, session_df_to_file, session_get_file_path, session_delete_file, parse_contents, render_upload_header, reset_button, continue_button, create_table_description, session_json_to_dict
from typing import Literal, Optional
from dash_pydantic_form import ModelForm, fields
import pandas as pd
import plotly.graph_objects as go
from plotly.graph_objs import Figure
from app.core.external.pages.sections.regression.data_preprocessing import preprocess_all_and_visualize
...
def create_prediction_plot(features_df: pd.DataFrame, predictions: list, prediction_column_name, type_selected) -> Figure:
    ...
    for i in range (0,len(columns)):
        if type_selected[i] == 'number':
            ...
            fig = px.scatter(sampled_df, x=col, y=prediction_column_name, template='simple_white')
...
def predict_and_visualize_regression_model(schema: PredictionFeatureSelectionSchema) -> dict:
    ...
    scaler = schema_load['scaler'][idx_target[0]]
    ...
    return dmc.Stack(
                [
                    ...
                    dmc.Button("Download Predicted Data", color="blue", id='regression-download_predicted_data_button', n_clicks=0,fullWidth=True),
...
def layout():
    ...
    layout = dmc.Stack(
        [
            ...
            dcc.Loading(type='circle',id='regression-loading-predict_data',style={"width": "80px", "height": "80px"}),
            dmc.Group(
                [
                    reset_button,
                    html.Div(
...
@callback(
    ...
    Output('regression-loading-predict_data','children',allow_duplicate=True),
