...
import flask
import uuid
from app.core.external.pages.sections.regression.utils import session_delete_file, parse_contents, render_upload_header, reset_button, continue_button, session_df_to_file, session_schema_create, render_upload_header_database, create_session, get_existing_well_list, get_well_data, get_well_data_list
...
upload_button_database = dmc.Button(
    id='regression-upload-data-database',
    children=dmc.Stack([
        dmc.Flex(DashIconify(icon="ic:outline-cloud-upload", height=30), justify='center'),
...
def layout():
    ...
    layout = dmc.Stack(
        [   
            ...
            dcc.Loading(type='circle',id='regression-loading-upload_data',style={"width": "80px", "height": "80px"}),
            dmc.Modal(id='regression-modal-database', opened=False),
            html.Div(
                id='regression-output-data'
            ),
            html.Div(
                id='regression-alert-delete'
            ), 
            dmc.Group(
                [
                    reset_button,
                    html.Div(id='regression-proceed-output')
                ],
...
@callback(
    ...
    Input('regression-input-select','value'),
...
@callback(
    ...
    Output('regression-upload-header', 'children', allow_duplicate=True),
    Output('regression-proceed-output', 'children', allow_duplicate=True),
    Output('regression-loading-upload_data','children',allow_duplicate=True),
    Output('regression-alert-delete','children',allow_duplicate=True),
        ...
        try:
            ...
            if len(check) < 1:
                upload_output = dmc.Alert(
                    'Input for Regression need at least one numeric column',
                    color="red",
                    variant="filled",
                    withCloseButton=True
                )
...
@callback(
    Output('regression-modal-database', 'opened'),
    Output('regression-modal-database', 'children'),
    Output('regression-output-data','children',allow_duplicate=True),
    Input('regression-upload-data-database', 'n_clicks'),
    [State('regression-input-select', 'value')],
    prevent_initial_call=True
)
def open_modal(n_clicks, selected_value):
    if selected_value == 'database' and n_clicks > 0:
        ...
        dmc.Space(h=20),
        dmc.Button("Apply", color="blue", id='regression-database-submit', n_clicks=0),]
        
        # Modal dengan form yang baru dibuat
        modal_database = dmc.Modal(
            title="Are you sure you want to reset?",
            id='regression-modal-database',
        )
...
@callback(
    ...
    Output('regression-proceed-output', 'children', allow_duplicate=True),
    Output('regression-loading-upload_data','children',allow_duplicate=True),
    Output('regression-modal-database', 'opened', allow_duplicate=True),
    Output('regression-alert-delete','children',allow_duplicate=True),
    Input('regression-database-submit','n_clicks'),
...
def upload_data_processing_database(n_clicks,form_data):
    if n_clicks > 0:
        ...
        if get_dataframe == None:
            ...
            upload_header = render_upload_header_database(f'database-{form_data['well_data_type']}', 'regression-rawData')
            ...
            check =turn_dataframe.select_dtypes(include=[np.number]).columns

            if len(check) < 1:
                upload_output = dmc.Alert(
                    'Input for regression need at least one numerical column',
                    color="red",
                    variant="filled",
                    withCloseButton=True
                )
...
@callback(
    ...
    Output('regression-upload-header', 'children',allow_duplicate=True),
    Output('regression-proceed-output','children',allow_duplicate=True),
    Input('regression-rawData-remove-data-database', 'n_clicks'),
    prevent_initial_call=True
)
