...
from app.core.external.pages.sections.clustering.utils import parse_validation_errors, session_df_to_file, session_get_file_path, session_delete_file, parse_contents, render_upload_header, reset_button, continue_button, create_table_description, session_json_to_dict
from typing import Literal, Optional
from dash_pydantic_form import ModelForm, fields
import pandas as pd
import plotly.graph_objects as go
from plotly.graph_objs import Figure
from app.core.external.pages.sections.clustering.data_preprocessing import preprocess_all_and_visualize
...
def predict_and_visualize_clustering_model(schema: PredictionFeatureSelectionSchema) -> dict:
    ...
    if isinstance(model, AgglomerativeClustering) or isinstance(model,DBSCAN):
        predictions = model.fit_predict(raw_df)
    else:
    ...
    print('Selesai Visualisasi')

    return dmc.Stack(
                [
                    dmc.SimpleGrid(visualization, cols=3),
                    dmc.Button("Download Predicted Data", color="blue", id='clustering-download_predicted_data_button', n_clicks=0,fullWidth=True),
...
def layout():
    ...
    layout = dmc.Stack(
        [
            ...
            dcc.Loading(type='circle',id='clustering-loading-predict_data',style={"width": "80px", "height": "80px"}),
            html.Div(
                id='clustering-alert-predict'
            ),
            dmc.Group(
                [
                    reset_button,
...
@callback(
    ...
    Output('clustering-loading-predict_data','children',allow_duplicate=True),
...
@callback(
    ...
    Output('clustering-alert-predict','children',allow_duplicate=True),
    Output('clustering-loading-predict_data','children',allow_duplicate=True),
...
def apply_prediction(n_clicks, form_data):
    if n_clicks > 0:
        ...
        try:
            ...
            result,model = predict_and_visualize_clustering_model(schema)
            
            output = result

            if isinstance(model, AgglomerativeClustering) or isinstance(model,DBSCAN):
                return output, continue_button, alert_predict, ""
            else:
