...
import flask
import uuid
from app.core.external.pages.sections.classification.utils import session_delete_file, parse_contents, render_upload_header, reset_button, continue_button, parse_contents_schema,session_get_file_path, session_df_to_file, render_upload_header_database, get_well_data, get_existing_well_list, get_well_data_list, create_session, reset_button_pred, session_delete_file_schema
import pandas as pd
...
upload_button_database = dmc.Button(
    id='predclass-upload-data-database',
    children=dmc.Stack([
        dmc.Flex(DashIconify(icon="ic:outline-cloud-upload", height=30), justify='center'),
...
button_prediction = dmc.Button("Predict", color="blue", id='predclass-apply_prediction', n_clicks=0,fullWidth=True)
button_downloaded = dmc.Button("Download Predicted Data", color="blue", id='predclass-apply_download', n_clicks=0,fullWidth=True)
...
def layout():
    ...
    layout = dmc.Stack(
        [   
            ...
            html.Div(
                id='predclass-upload-header',
            ),
            html.Div(
                id='predclass-output-data'
            ), 
            html.Div(
                [
                ...
                id='predclass-upload-header-schema',
                style={'margin-top':'2px'},
            ),
            html.Div(
                id='predclass-output-schema'
            ),
            html.Div(
                id='predclass-proceed-prediction-button'
            ),
            dcc.Loading(type='circle',id='predclass-loading-predict_data',style={"width": "80px", "height": "80px"}),
            html.Div(
                id='predclass-proceed-output'
            ),
            html.Div(
                id='predclass-download-prediction',
            ),
            html.Div(
                reset_button_pred,
                id='predclass-reset-prediction',
            ),
            dmc.Modal(id='predclass-modal-database', opened=False),
        ],
        gap='md'
    )
...
@callback(
    Output('predclass-upload-header','children',allow_duplicate=True),
    Output('predclass-output-data','children',allow_duplicate=True),
    Output('predclass-proceed-output','children',allow_duplicate=True),
    Input('predclass-input-select','value'),
...
@callback(
    Output('predclass-output-data', 'children', allow_duplicate=True),
    Output('predclass-upload-header', 'children', allow_duplicate=True),
    Output('predclass-loading-predict_data','children',allow_duplicate=True),
    Input('predclass-upload-data', 'contents'),
    State('predclass-upload-data', 'filename'),
    prevent_initial_call=True
)
def upload_data_processing(contents, filename):
    if contents is not None:
        ...
        try:
            ...
            df = parse_contents(contents, filename, 'predclass-rawData', False)
            upload_header = render_upload_header(filename, 'predclass-rawData')
...
@callback(
    Output('predclass-modal-database', 'opened'),
    Output('predclass-modal-database', 'children'),
    Output('predclass-output-data','children',allow_duplicate=True),
    Input('predclass-upload-data-database', 'n_clicks'),
    [State('predclass-input-select', 'value')],
    prevent_initial_call=True
)
def open_modal(n_clicks, selected_value):
    if selected_value == 'database' and n_clicks > 0:
        ...
        dmc.Space(h=20),
        dmc.Button("Apply", color="blue", id='predclass-database-submit', n_clicks=0),]
        
        # Modal dengan form yang baru dibuat
        modal_database = dmc.Modal(
            title="Are you sure you want to reset?",
            id='predclass-modal-database',
        )
...
@callback(
    Output('predclass-output-data', 'children', allow_duplicate=True),
    Output('predclass-upload-header', 'children', allow_duplicate=True),
    Output('predclass-modal-database', 'opened', allow_duplicate=True),
    Input('predclass-database-submit','n_clicks'),
...
def upload_data_processing_database(n_clicks,form_data):
    if n_clicks > 0:
        ...
        if get_dataframe == None:
            ...
            upload_header = render_upload_header_database(f'database-{form_data['well_data_type']}', 'predclass-rawData')

            session_df_to_file(turn_dataframe, 'predclass-rawData')
...
@callback(
    Output('predclass-output-schema', 'children', allow_duplicate=True),
    Output('predclass-upload-header-schema', 'children', allow_duplicate=True),
    Output('predclass-proceed-prediction-button', 'children', allow_duplicate=True),
    Input('predclass-upload-schema', 'contents'),
    State('predclass-upload-schema', 'filename'),
    prevent_initial_call=True
)
def upload_schema_process(contents, filename):
    ...
    if contents is not None:
        ...
        try:
            ...
            schema = parse_contents_schema(contents, filename, 'predclass-SchemaUse')
            header_schema = render_upload_header(filename,'predclass-schemaUse')
...
@callback(
    Output('predclass-proceed-output', 'children', allow_duplicate=True),
    Output('predclass-download-prediction','children',allow_duplicate=True),
    Output('predclass-loading-predict_data','children',allow_duplicate=True),
    Input('predclass-apply_prediction', 'n_clicks'),
    prevent_initial_call=True
)
    ...
    if n_clicks > 0:
        ...
        try:
            ...
            if upload_filename.endswith('.joblib'):
                schema = joblib.load(session_get_file_path('predclass-SchemaUse', extension='joblib'))
            else:
                schema = joblib.load(session_get_file_path('predclass-SchemaUse', extension='pkl'))

            df_1 = pd.read_csv(session_get_file_path('predclass-rawData', extension='csv'), index_col=0)
            df = df_1.copy()
            df.reset_index(inplace=True)
            ...
            encode = schema['label_enc'][idx_target[0]]
            ...
            return dmc.Stack(
                    [
                        ...
                        dcc.Download(id="predclass-download-predicted-data"),
                    ]
                ), button_downloaded, ""
...
@callback(
    Output('predclass-output-data', 'children'),
    Output('predclass-upload-header', 'children'),
    Input('predclass-rawData-remove-data', 'n_clicks'),
)
def remove_data(n_clicks):

    if n_clicks > 0:
        session_delete_file('predclass-rawData')
        # reset_session()
...
@callback(
    Output('predclass-output-data', 'children', allow_duplicate=True),
    Output('predclass-upload-header', 'children', allow_duplicate=True),
    Input('predclass-rawData-remove-data-database', 'n_clicks'),
    prevent_initial_call=True
)
...
@callback(
    ...
    Output('predclass-upload-header-schema', 'children', allow_duplicate=True),
    Output('predclass-proceed-prediction-button', 'children', allow_duplicate=True),
    Input('predclass-schemaUse-remove-data', 'n_clicks'),
    prevent_initial_call=True
)
def remove_schema(n_clicks):
    ...
    if n_clicks > 0:    
        ...
        if upload_filename.endswith('.joblib'):
            session_delete_file_schema('predclass-SchemaUse','joblib')
        else:
            session_delete_file_schema('predclass-SchemaUse','pkl')
...
@callback(
    Output("predclass-download-predicted-data", "data"),
    Input('predclass-apply_download', 'n_clicks'),
    prevent_initial_call=True
)
