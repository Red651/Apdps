...
options_map = {
    'severity': Severity,
    'workover_job_category': WorkoverJobCategory,
    'wellservice_job_category': WellServiceJobCategory,
    'contract_type': ContractType,
    'rig_type': RigType,
    'hazard_type': HazardType,
    'job_document_type': JobDocumentType,
    'operation_code': OperationCode,
    'well_status': WellStatus,
    'bha_component_type': BHAComponentType,
    'mud_type': MudType,
    "well_status": WellStatus,
    "well_type": WellType,
...
def save_upload_file(db: Session, upload_file: UploadFile, user) -> FileInfo:
    ...
    output = FileInfo.model_validate(db_file)

    return output

def save_upload_physical_item(db: Session, upload_file: UploadFile, user) -> FileInfo:
...
def save_upload_multiple_files(db: Session, upload_files: List[UploadFile], user) -> List[FileInfo]:
    ...
    for file_obj in file_objs:
        output = FileInfo.model_validate(file_obj).model_dump()
        output['file_remove_path'] = f"/utils/delete/file/{db_file.id}"
        output['file_download_path'] = f"/utils/download/file/{db_file.id}"
...
def jsonify_and_save_tabular_file(db: Session, file: UploadFile, user):
    ...
    return {
        'file_info':output, 
        'data':TabularData(headers=df.columns.to_list(), records=records)
    }
