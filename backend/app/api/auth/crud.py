...
def get_users_by_kkks(db: Session, kkks_id: str):
...
def get_kkks_by_id(db: Session, kkks_id: str):
    return db.query(models.KKKS).filter(models.KKKS.id == kkks_id).first()

def get_kkks(db: Session):
    return db.query(models.KKKS.name, models.KKKS.id.label('value')).all()