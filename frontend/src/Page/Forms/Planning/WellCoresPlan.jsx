...
const WellCoresFormPlann = () => {
  const { state, dispatch } = useJobContext();
  const toast = useToast();
  const data = state?.initialOperationData?.actual_job?.well?.well_cores;
  const clearFileInput = useRef(null);
  const [tableData, setTableData] = useState([]);
  const [formData, setFormData] = useState({
    unit_type:
      state?.initialOperationData?.actual_job?.well?.unit_type || "METRICS",
    top_depth: null,
    base_depth: null,
    core_diameter: null,
    core_type: null,
  ...
  const handleAddData = useCallback(async () => {
    ...
    try {
      ...
      if (uploadResponse.data.success) {
        ...
        setFormData({
          unit_type:
            state?.initialOperationData?.actual_job?.well?.unit_type ||
            "METRICS",
          top_depth: "",
          base_depth: "",
          image_file_id: "",
...
export default WellCoresFormPlann;
