...
import { UploadFileTabular } from "../Operation/FormHandling/Utils/UploadFileTabular";
import { useParams } from "react-router-dom";
import { all } from "axios";

const NewFileUploadTabular = ({
  acceptFormat = [],
  onChange = () => {},
  initialData = null,
  title = "Upload File",
  ...
  const validateFile = React.useCallback((file) => {
    ...
    if (fileSizeInMB > maxFileSize) {
      ...
      // console.log("File kebeasararnm");
      
    }
    const fileExtension = `.${file.name.split(".").pop()}`.toLowerCase();
    if (acceptFormat.length > 0 && !acceptFormat.includes(fileExtension)) {
      throw new Error(
        `File type not allowed. Allowed types: ${acceptFormat.join(", ")}`
      );
  ...
  const handleReplaceFile = async () => {
    if (blobFile) {
      ...
      try {
        ...
        const response = await UploadFileTabular(blobFile);
  ...
  const handleSelectedFile = async (e) => {
    ...
    if (selectedFile) {
      ...
      try {
        validateFile(selectedFile);
        const response = await UploadFileTabular(selectedFile);
        if (response.status === 200) {
          setFileData({
            file_id: response.data.file_id,
            filename: response.data.filename,
...
export default NewFileUploadTabular;
