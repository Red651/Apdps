...
const WellDocuments = () => {
  ...
  const initialLogsData = state?.wellMaster?.well_documents || [];
  ...
  const handleAddData = useCallback(async () => {
    ...
    try {
      ...
      resetForm();
      showToast("Success", editIndex !== null ? "Document updated" : "Document added", "success");
    } catch (error) {
      const errorMessage =
        ...
        <GridItem>
          ...
          >
            <Flex gap={6} direction="column">
              ...
              {editIndex !== null && (
                <Text backgroundColor={"yellow.100"} p={2} mt={2}>Current File: {formData.filename}</Text>
              )}
            </Flex>
            <Flex mt={4} justifyContent="flex-end" gap={2}>
              <Button
                ...
                isLoading={isLoading}
                leftIcon={editIndex !== null ? <Icon as={IconEdit} /> : <Icon as={IconPlus} />}
                borderRadius={"full"}
              >
