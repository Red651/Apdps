...
import { Tabs, TabList, TabPanels, Tab, TabPanel, Flex, Spinner, Text, Heading, Alert, AlertIcon, TabIndicator } from "@chakra-ui/react";
...
const ViewExploration = () => {
  ...
  const geoCenter = geo?.center && geo.center.x != null && geo.center.y != null
    ...
    <Flex w="84vw" fontFamily={"Mulish"} direction="column" p={4}>
      <Flex alignItems={"center"} justifyContent={"space-between"}>
        <Box mt={3} mb={8}>
          ...
          <Text fontSize='lg' color={"gray.400"}>{jobType}</Text>
        </Box>
        <Box>
        <Alert status={getStatus(status)} variant={status == "FINISHED" ? "solid" : "subtle"} p={5} borderRadius="10px">
          <AlertIcon />
            {status}
          </Alert>
        </Box>
