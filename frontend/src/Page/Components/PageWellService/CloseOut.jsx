...
const CloseOutExplorationKKKS = () => {
  ...
  useEffect(() => {
    ...
    try {
      const fetchData = async () => {
        const data = await getTableKKKS(kkks_id, "wellservice", "co");
        setTableData(data.data);
      };

      const fetchCountData = async () => {
        const data = await getCountDataSummary(kkks_id, "wellservice", "co");
  ...
  const handleOperate = async () => {
    setLoading(true);
    try {
      const updatedData = await getTableKKKS(kkks_id, "wellservice", "co");
            ...
            <Flex>
              {jobs.STATUS === "PROPOSED" ? (
                <IconButton
                  ...
                  colorScheme="green"
                  size="sm"
                  to={`/wellservice/closeout/form/${jobs.job_id}/`}
                  rounded="full"
                  aria-label="Propose"
                />
